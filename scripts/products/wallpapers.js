const wallpaper = () => {
  const produtctsRowWallpaper = document.querySelector(
    ".produtcts__row_wallpaper"
  );

  const changeTitle = (count) => {
    document.querySelector(
      ".produtcts__count_wallpaper"
  ...
  const fetchData = async () => {
    const response = await fetch(
      "https://products-catalog-7b75d-default-rtdb.firebaseio.com/db/wallpaper.json"
    );
